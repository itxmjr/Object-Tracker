            ...
            >
                ...
                <div className="grid gap-1.5">
                    {payload.map((item, index) => {
                        ...
                        const itemConfig = getPayloadConfigFromPayload(config, item, key);
                        const indicatorColor = color || (itemPayload.payload as Record<string, unknown>)?.fill || itemPayload.color;
                        ...
                        return (
                            ...
                            >
                                {formatter && itemPayload?.value !== undefined && itemPayload.name ? (
                                    formatter(itemPayload.value, String(itemPayload.name), item, index, itemPayload.payload as Record<string, unknown>)
                                ) : (
                                    <>
                                        {itemConfig?.icon ? (
                                            <itemConfig.icon />
                                        ...
                                        >
                                            <div className="grid gap-1.5">
                                                ...
                                                <span className="text-muted-foreground">
                                                    {itemConfig?.label || String(itemPayload.name)}
                                                </span>
                                            </div>
                                            {itemPayload.value != null && (
                                                <span className="font-mono font-medium tabular-nums text-foreground">
                                                    {Number(itemPayload.value).toLocaleString()}
                                                </span>
                                            )}
                                        </div>
                                    </>
